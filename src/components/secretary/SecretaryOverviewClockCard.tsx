...
import { attendanceSettled, secretaryApi } from "@/lib/api";
import {
  formatExpectedClock,
  formatLagosClock,
  mapCheckInWindow,
...
import styles from "@/components/employee/EmployeeOverviewClockCard.module.css";
...
export function SecretaryOverviewClockCard() {
  ...
  const { data, loading, refetch } = useAsyncData(
    () => attendanceSettled(secretaryApi.attendance.status()),
    [],
  );
  ...
  async function handleClockIn() {
    ...
    try {
      await runAction(
        "Clock in",
        async () => {
          const payload = await secretaryApi.attendance.clockIn({});
          const fromApi = formatClockInTime(mapCheckInWindow(payload).checkInAt);
          setRecordedClockIn(
            fromApi || formatLagosClock(new Date().toISOString()),
          );
          refetch();
        },
        "Clock-in recorded",
  ...
  async function handleClockOut() {
    ...
    try {
      await runAction(
        "Clock out",
        async () => {
          await secretaryApi.attendance.clockOut({});
          refetch();
        },
        "Clock-out recorded",
      );
